...
const ActivityAwareCollection = require("./util/ActivityAwareCollection");
...
class Client extends EventEmitter {
    ...
    */
    constructor(token, options) {
        ...
        this.users = new ActivityAwareCollection(User);
        this.presence = {
            game: null,
            status: "offline"
        };
    ...
    */
    ...
    sweepCaches(ttl){
        let usersSweeped = 0;
        let membersSweeped = 0;
        for(const user of this.users.values()){
            if(user.id === this.user.id) continue;
            const minutesInactive = (Date.now() - user.lastUpdated)/(1000*60);
            if(minutesInactive > ttl) {
                usersSweeped++;
                this.users.remove({id: user.id});
            }
        }
        for(const guild of this.guilds.values()){
            for(const member of guild.members.values()){
                if(member.id === this.user.id) continue;
                const minutesInactive = (Date.now() - member.lastUpdated)/(1000*60);
                console.log(`${member.id}|${guild.id}|${minutesInactive}`);
                if(minutesInactive > ttl) {
                    membersSweeped++;
                    guild.members.remove({id: member.id});
                }
            }
        }
        return {
            users: usersSweeped,
            members: membersSweeped
        };
    }
