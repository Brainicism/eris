...
class VoiceConnection extends EventEmitter {
  ...
  connect(data) {
    ...
    this.connectionTimeout = setTimeout(
      ...
      this.shard.client
        ? this.shard.client.shards.options.connectionTimeout
        : 30000
    ).unref();
    if (!data.endpoint) {
    ...
    if (!data.token || !data.session_id || !data.user_id) {
      this.disconnect(
        new Error("Malformed voice server update: " + JSON.stringify(data))
      );
      return;
    }
    ...
    this.ws.on("open", () => {
      ...
      if (
        ...
        lastEndpoint === this.endpoint.hostname
      ) {
    ...
    this.ws.on("message", (m) => {
      ...
      switch (packet.op) {
        case VoiceOPCodes.READY: {
          ...
          const availableMode = SUPPORTED_ENCRYPTION_MODES.find(
            (mode) =>
              packet.d.modes.includes(mode) &&
              (mode === "aead_aes256_gcm_rtpsize" ? aes256Available : true)
          );
          if (!availableMode) {
        ...
        case VoiceOPCodes.SESSION_DESCRIPTION: {
          ...
          this.#nonceBuffer = Buffer.alloc(
            this.mode === "aead_aes256_gcm_rtpsize" ? 12 : 24
          );
          this.connecting = false;
          this.reconnecting = false;
          this.ready = true;
  ...
  registerReceiveEventHandler() {
    this.udpSocket.on("message", (msg) => {
      ...
      if (rtpVersion !== 2) {
        ...
        this.emit(
          "warn",
          "Recieved packet with an invalid RTP version: " + rtpVersion
        );
        return;
      }
      ...
      if (this.mode === "aead_aes256_gcm_rtpsize") {
        ...
        data = Buffer.concat([
          decipher.update(msg.subarray(headerSize, msg.length - 20)),
          decipher.final(),
        ]);
      } else if (Sodium) {
        data = Buffer.alloc(
      ...
      if (hasExtension) {
        ...
        if (msg[extOffset + 0] === 0xbe && msg[extOffset + 1] === 0xde) {
      ...
      if (this.receiveStreamOpus) {
        ...
        this.receiveStreamOpus.emit(
          "data",
          data,
          this.ssrcUserMap[msg.readUIntBE(8, 4)],
          msg.readUIntBE(4, 4),
          msg.readUIntBE(2, 2)
        );
      }
  ...
  _sendAudioFrame(frame) {
    ...
    if (this.mode === "aead_aes256_gcm_rtpsize") {
      ...
      return this.sendUDPPacket(
        this.sendBuffer.subarray(0, headerSize + result.length + 4)
      );
    } else if (Sodium) {
      const ABYTES = Sodium.crypto_aead_xchacha20poly1305_ietf_ABYTES;
      const length = frame.length + ABYTES;
      this.sendBuffer.fill(
        0,
        headerSize + frame.length,
        headerSize + frame.length + ABYTES
      );
      ...
      return this.sendUDPPacket(
        this.sendBuffer.subarray(0, headerSize + length + 4)
      );
    }
  }
