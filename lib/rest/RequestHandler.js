...
class RequestHandler {
  ...
  ratelimits = {};
  readyQueue = [];
  userAgent = `DiscordBot (https://github.com/projectdysnomia/dysnomia, ${
          ...
          if (file) {
            if (Array.isArray(file)) {
              ...
              file.forEach(function (f) {
                ...
                if (attachedFieldName === "payload_json") {
                  // Use the body parameter to supply payload_json instead
                  return;
                }
                ...
                if (body[key] != null) {
                  if (Array.isArray(body[key])) {
                    body[key].forEach(function (val) {
                      qs += `&${encodeURIComponent(key)}=${encodeURIComponent(
                        val
                      )}`;
                    });
                  } else {
        ...
        req.once("response", (resp) => {
          ...
          _respStream
            ...
            .once("end", () => {
              ...
              if (
                method !== "GET" &&
                (resp.headers["x-ratelimit-remaining"] == null ||
                  resp.headers["x-ratelimit-limit"] == null) &&
              ...
              if (resp.statusCode !== 429) {
                const content =
                  typeof body === "object" ? `${body.content} ` : "";
                this.#client.emit(
                  "debug",
                ...
                if (resp.statusCode === 429) {
                  ...
                  this.#client.emit(
                    ...
                    `${
                      ...
                      this.ratelimits[route].remaining
                    }/${this.ratelimits[route].limit} left | Reset ${delay} (${
                      this.ratelimits[route].reset - now
                    }ms left) | Scope ${resp.headers["x-ratelimit-scope"]}`
                  );
                  if (delay) {
                    setTimeout(() => {
                      cb();
